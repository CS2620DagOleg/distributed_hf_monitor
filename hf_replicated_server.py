...
# Import heart failure monitoring extension
from hf_server_extension import (
    initialize_hf_db,
    alert_monitor_loop,
    SendRiskReport,
    ListRiskReports,
    handle_risk_report_replication,
    handle_update_alert_sent_replication,
    get_risk_reports_for_state_transfer,
    apply_risk_reports_from_state
)
...
class HeartFailureMonitoringServer(chat_pb2_grpc.ChatServiceServicer):
    def __init__(self, config):
        ...
        self.initialize_db()
        self.initialize_hf_db()  # Initialize heart failure monitoring tables
    ...
    # Method from hf_server_extension.py
    initialize_hf_db = initialize_hf_db
    alert_monitor_loop = alert_monitor_loop
    SendRiskReport = SendRiskReport
    ListRiskReports = ListRiskReports

    def initialize_db(self):
        # Original chat database initialization
        self.cursor.execute('''
    ...
    def start_election(self):
        ...
        if not lower_id_found:
            ...
            # Start the alert monitor loop if we're the leader
    ...
    def ReplicateOperation(self, request, context):
        ...
        try:
            ...
            # Handle heart failure monitoring operations
            elif op_type == "risk_report":
                handle_risk_report_replication(self, data)
            elif op_type == "update_alert_sent":
                handle_update_alert_sent_replication(self, data)
    ...
    def join_cluster(self):
        # Modified to include risk reports in state transfer
        try:
            ...
            if resp.success:
                ...
                self.cursor.execute("DELETE FROM risk_reports")
                
                # Apply accounts and messages
                ...
                apply_risk_reports_from_state(self, risk_reports)
    ...
    def JoinCluster(self, request, context):
        ...
        # Get risk reports
        risk_reports = get_risk_reports_for_state_transfer(self)
...
def serve():
    ...
    chat_service = HeartFailureMonitoringServer(config)
    ...
    print("Heart Failure Monitoring Enabled")
    try:
        while True:
            time.sleep(86400)
